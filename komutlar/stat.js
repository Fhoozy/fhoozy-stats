...
let cDurum = member.presence?.status;
    ...
    .setFooter(message.guild.name + ` • ` + message.author.tag + ` tarafından istendi. `, message.guild.iconURL())
    ...
    **\`•\`Ses: ${voiceData[0] ? `<#${voiceData[0].channelID}>` : 'Veri Yok!'}: \`${voiceData[0] ? moment.duration(voiceData[0].totalTime).format("HH [Saat], mm [Dakika]") : 'Veri Yok!'}\`**
     ...
     __**Son 14 Gün içindeki kullanıcı ses ve Mesaj istatistikleri.**__
     `).addFields({
        name: `**__Mesaj__:**`,
